...
import { getId } from "../store/selectors.ts";
...
export default function GameOver({gameMatrix, onStart,score,setGameMatrix, setCurrentScore, isGameOver, setIsGameOver}:GameOverProps) {
    ...
    useEffect(() => {
        if(isGameOver){
            axios.post('http://localhost:4444/updateBestScore', {
                id,
                score
            })
            dispatch(userAction.setBestScore(score));
        }
    })
    ...
    return (
        <div className="border-customGrey border-8 rounded-2xl relative z-4" style={gridContainerStyle}>
            ...
            <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 w-screen">
                <h2 className="font-extrabold text-6xl">Игра закончена!</h2>
                <NewGameButton onStart={onStart} setGameMatrix={setGameMatrix} size={gameMatrix.length} setCurrentScore={setCurrentScore} buttonMessage={"Попробовать снова"} setIsGameOver={setIsGameOver}/>
            </div>
        </div>
    );
}
